...
F-Droid是由歐洲非營利組織運營的應用程式商店。跟Google Play以及其他應用程式商店不同，F-Droid只接受上架自由軟體APP。而且APK原始碼必須由F-Droid伺服器建置與簽名，不接受上傳事先編譯好的APK。
...
Android手機的瀏覽器陣營大致可以分為Chromium系(Blink)與Firefox系(Gecko)。
...
|[Tor Browser for Android](https://support.torproject.org/tormobile/tormobile-7/)|支援Tor連線的瀏覽器，基於Firefox開發。需要啟用Guardian Project套件庫。|
